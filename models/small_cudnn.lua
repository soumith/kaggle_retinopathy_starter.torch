function createModel(nGPU)
   ...
   features:add(cudnn.SpatialConvolution(3,32,11,11,4,4,2,2))       -- 224 -> 55
   features:add(nn.SpatialBatchNormalization(32,1e-3))
   features:add(cudnn.ReLU(true))
   features:add(cudnn.SpatialMaxPooling(3,3,2,2))                   -- 55 ->  27
   features:add(cudnn.SpatialConvolution(32,48,5,5,1,1,2,2))       --  27 -> 27
   features:add(nn.SpatialBatchNormalization(48,1e-3))
   features:add(cudnn.ReLU(true))
   features:add(cudnn.SpatialMaxPooling(3,3,2,2))                   --  27 ->  13
   features:add(cudnn.SpatialConvolution(48,64,3,3,1,1,1,1))      --  13 ->  13
   features:add(nn.SpatialBatchNormalization(64,1e-3))
   features:add(cudnn.ReLU(true))
   features:add(cudnn.SpatialConvolution(64,48,3,3,1,1,1,1))      --  13 ->  13
   features:add(nn.SpatialBatchNormalization(48,1e-3))
   features:add(cudnn.ReLU(true))
   features:add(cudnn.SpatialConvolution(48,48,3,3,1,1,1,1))      --  13 ->  13
   features:add(nn.SpatialBatchNormalization(48,1e-3))
   features:add(cudnn.ReLU(true))
   ...
   for i=1,nGPU do
      ...
      s:add(nn.Linear(48*6*6, 256/nGPU))
      s:add(nn.BatchNormalization(256/nGPU,1e-3))
      s:add(nn.ReLU())
      branch1:add(s)
   end

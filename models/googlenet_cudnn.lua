...
local function inception(input_size, config)
   ...
   if config[1][1] ~= 0 then
      ...
      conv1:add(nn.SpatialBatchNormalization(config[1][1], nil, opt.bnMomentum)):add(cudnn.ReLU(true))
      concat:add(conv1)
   end

   local conv3 = nn.Sequential()
   conv3:add(cudnn.SpatialConvolution(  input_size, config[2][1],1,1,1,1))
   conv3:add(nn.SpatialBatchNormalization(config[2][1], nil, opt.bnMomentum)):add(cudnn.ReLU(true))
   conv3:add(cudnn.SpatialConvolution(config[2][1], config[2][2],3,3,1,1,1,1))
   conv3:add(nn.SpatialBatchNormalization(config[2][2], nil, opt.bnMomentum)):add(cudnn.ReLU(true))
   concat:add(conv3)

   local conv3xx = nn.Sequential()
   conv3xx:add(cudnn.SpatialConvolution(  input_size, config[3][1],1,1,1,1))
   conv3xx:add(nn.SpatialBatchNormalization(config[3][1], nil, opt.bnMomentum)):add(cudnn.ReLU(true))
   conv3xx:add(cudnn.SpatialConvolution(config[3][1], config[3][2],3,3,1,1,1,1))
   conv3xx:add(nn.SpatialBatchNormalization(config[3][2], nil, opt.bnMomentum)):add(cudnn.ReLU(true))
   ...
   if config[4][2] ~= 0 then
      ...
      pool:add(nn.SpatialBatchNormalization(config[4][2], nil, opt.bnMomentum)):add(cudnn.ReLU(true))
   end
   concat:add(pool)
...
function createModel(nGPU)
   ...
   features:add(nn.SpatialBatchNormalization(64,nil,opt.bnMomentum)):add(cudnn.ReLU(true))   
   ...
   features:add(nn.SpatialBatchNormalization(192, nil, opt.bnMomentum)):add(cudnn.ReLU(true))
   features:add(cudnn.SpatialMaxPooling(3,3,2,2):ceil())
   ...
   main_branch:add(nn.Dropout(0.4))
   main_branch:add(cudnn.SpatialConvolution(1024,1024,2,2,2,2))
   main_branch:add(nn.SpatialBatchNormalization(1024, nil, opt.bnMomentum))
   ...
   main_branch:add(nn.Linear(1024,5))
   ...
   aux_classifier:add(nn.SpatialBatchNormalization(128, nil, opt.bnMomentum))
   aux_classifier:add(nn.View(128*4*4):setNumInputDims(3))
   aux_classifier:add(nn.Linear(128*4*4,768))
   aux_classifier:add(nn.BatchNormalization(768, nil, opt.bnMomentum))
   aux_classifier:add(nn.ReLU())
   aux_classifier:add(nn.Linear(768,5))

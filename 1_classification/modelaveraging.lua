...
opt = {
   ...
   bestAccuracy = 0,
   test='',
   testModelDir='results/all_models/',
   testModel='results/googlenet_cudnn_attempt3/model_11.t7',
   retrain='',
...
function testBatch(id,inputGPU,filenames,modelFile,outputs)
    collectgarbage()
    local diskModel = torch.load(modelFile)
    local model = diskModel.model
    model:cuda()
    model:evaluate()
    outputs[id] = torch.exp(model:forward(inputGPU))
end
...
for batchNumber = 1,numBatches do
    ...
    inputs, filenames = getTestingMiniBatch(indexStart, indexEnd)

    local inputGPU = torch.CudaTensor()
    inputGPU:resize(inputs:size()):copy(inputs)

    local modelFiles = dir.getfiles(opt.testModelDir)

    local donkeys = threads.Threads(#modelFiles,function(threadid) print('Starting Thread with id: '..threadid) end)
    outputs = tds.hash()
    for _,modelFile in ipairs(modelFiles) do
    	donkeys:addjob(function return __threadid,inputGPU,filenames,modelFile,outputs end, testMiniBatch)
    end

    donkeys:synchronize()
    local outputBatch = torch.CudaTensor(batchElems,5):zero()
    for i=1,#outputs do
    	outputBatch:add(outputs[i])
    end
    outputBatch:div(#outputs)
...
local df = torch.DiskFile('predictions_googlenet_cudnn_attempt3_average','rw')

function createModel(nGPU)
   ...
   features:add(nn.SpatialBatchNormalization(96,nil,opt.bnMomentum))
   features:add(cudnn.ReLU(true))
   features:add(cudnn.SpatialMaxPooling(2, 2, 2, 2))

   features:add(cudnn.SpatialConvolution(96, 256, 5, 5, 1, 1))
   features:add(nn.SpatialBatchNormalization(256,nil,opt.bnMomentum))
   features:add(cudnn.ReLU(true))
   features:add(cudnn.SpatialMaxPooling(2, 2, 2, 2))

   features:add(cudnn.SpatialConvolution(256, 512, 3, 3, 1, 1, 1, 1))
   features:add(nn.SpatialBatchNormalization(512,nil,opt.bnMomentum))
   features:add(cudnn.ReLU(true))

   features:add(cudnn.SpatialConvolution(512, 1024, 3, 3, 1, 1, 1, 1))
   features:add(nn.SpatialBatchNormalization(1024,nil,opt.bnMomentum))   
   features:add(cudnn.ReLU(true))

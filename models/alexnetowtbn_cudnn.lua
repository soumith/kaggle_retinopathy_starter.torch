function createBranch(nInputPlanes,nGPU)
   local branch
   if nGPU == 1 then
      branch = nn.Concat(2)
   else
      branch = nn.ModelParallel(2)
   end
   for i=1,nGPU do
      local s = nn.Sequential()
      s:add(nn.Linear(nInputPlanes, 4096/nGPU))
...
function createModel(nGPU)
   ...
   classifier:add(createBranch(256*6*6,nGPU))
   classifier:add(createBranch(4096,nGPU))
